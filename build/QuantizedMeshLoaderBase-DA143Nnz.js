...
class ct extends st {
  ...
  parse(t) {
    ...
    }, c = r(), $ = a(c, Uint16Array), q = a(c, Uint16Array), J = a(c, Uint16Array), g = new Float32Array(c), R = new Float32Array(c), P = new Float32Array(c);
    let C = 0, S = 0, I = 0;
    const U = 32767;
    for (let n = 0; n < c; ++n)
      C += v($[n]), S += v(q[n]), I += v(J[n]), g[n] = C / U, R[n] = S / U, P[n] = I / U;
    const M = c > 65536, m = M ? Uint32Array : Uint16Array;
    ...
    const x = (n, u) => R[u] - R[n], X = (n, u) => -x(n, u), E = (n, u) => g[n] - g[u], Z = (n, u) => -E(n, u), Y = r(), B = a(Y, m);
    B.sort(x);
    const j = r(), z = a(j, m);
    z.sort(E);
    const K = r(), F = a(K, m);
    ...
    for (; e < s.byteLength; ) {
      ...
      if (n === 1) {
        ...
        for (let d = 0; d < c; d++) {
          ...
          const y = 1 - (Math.abs(k) + Math.abs(T));
          if (y < 0) {
            const V = k;
            k = (1 - Math.abs(T)) * H(V), T = (1 - Math.abs(V)) * H(T);
          }
          const L = Math.sqrt(k * k + T * T + y * y);
          p[3 * d + 0] = k / L, p[3 * d + 1] = T / L, p[3 * d + 2] = y / L;
        }
        A.octvertexnormals = {
          extensionId: n,
          normals: p
...
//# sourceMappingURL=QuantizedMeshLoaderBase-DA143Nnz.js.map
